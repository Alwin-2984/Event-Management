...
public class TokenGenerator {
  ...
  public static final class VerificationStatus {
    ...
    private VerificationStatus(String data, String roleId, long time, long expiry) {
      this.data = data;
      this.roleId = roleId;
      this.time = time;
      this.expiry = expiry;
  ...
  private static final Pattern PURPOSE_PATTERN = Pattern.compile("\\w+");
  private static final String TOKEN_CONTENT_INVALID = "Token content is invalid";
  ...
  /**
   * Creates an instance of TokenGenerator with the given password and salt. The
   * password cannot be null and must be exactly 16 ASCII characters. The salt
   * also cannot be null and must be exactly 16 digit hexadecimal value.
   *
  ...
  /**
   * Create a token with the given purpose, data, expiry. The token will have an
   * embedded timestamp only if the expiry is not {@code null} and is &gt; 0
   * milliseconds. The purpose should only contain the characters 'a-Z', 'A-Z',
   * '0-9' and '_'.
   *
   * @param purpose the tag for identifying the purpose of the token - the same
   *                value has to be passed for verifying the token
   * @param data    the data to be embedded in the token - cannot be null
   * @param roleId  the role to be embedded in the token - cannot be null
   * @param expiry  the time in seconds for the expiry of the token - if this
   *                value is not {@code null} and is &gt; 0 milliseconds, a
   *                timestamp will be embedded in the token which will be used
   *                during the verification of the token
   *
   * @return the generated token and its related information
   *
   * @throws IllegalArgumentException if there is a problem with the given purpose
   *                                  or data
   */
  public Token create(String purpose, String data, String roleId, Duration expiry) throws IllegalArgumentException {
    ...
    String token = purpose + SEPARATOR + encode(data) + SEPARATOR + encode(roleId) + SEPARATOR + t + SEPARATOR + e
        + SEPARATOR + r;
  ...
  /**
   * Verifies the given token. The purpose should match the value given during the
   * creation of the token. The token will be checked for expiration if the token
   * was created with an expiry value &gt; 0. The separator used will be
   * {@link #SEPARATOR}.
   *
   ...
   */
  public VerificationStatus verify(String purpose, String token) throws InvalidTokenException, TokenExpiredException {
    return verify(purpose, token, true);
  }

  /**
   * Verifies the given token. The purpose should match the value given during the
   * creation of the token. The token will be checked for expiration only if the
   * token was created with an expiry value &gt; 0 and checkExpiry is
   * {@code true}.
   *
   * @param purpose     the tag for identifying the purpose of the token - should
   *                    match the value given during the creation of the token
   * @param token       the token to be verified
   * @param checkExpiry flag specifying whether the token should be checked for
   *                    expiry even if it contains an embedded timestamp
   *
   * @return the verification status
   */
  public VerificationStatus verify(String purpose, String token, boolean checkExpiry)
    ...
    if (parts.length != 6 || !parts[0].equals(purpose)) {
      throw new InvalidTokenException(TOKEN_CONTENT_INVALID);
    }

    long keyTime;
    long expiry;
    try {
      keyTime = Long.parseLong(parts[3]);
      expiry = Long.parseLong(parts[4]);
    } catch (NumberFormatException e) {
      throw new InvalidTokenException(TOKEN_CONTENT_INVALID, e);
    }
    ...
    return new VerificationStatus(decode(parts[1]), decode(parts[2]), keyTime, keyTime + expiry);
  }
}
