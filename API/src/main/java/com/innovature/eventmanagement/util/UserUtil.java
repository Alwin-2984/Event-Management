...
public class UserUtil {
  ...
  // Checks if user is having otp verified status
  ...
  // Checking active status and returns user
  public User checkActiveStatus(String email) {
    ...
    if (user != null) {
      ...
      if (status == Status.ACTIVE.value) {
        ...
        // User is INACTIVE, throw BadRequestException with user blocked message
        ...
        // User is DELETED, throw BadRequestException with user deleted message
  ...
  // Generate Token for existing user for handling google signin
