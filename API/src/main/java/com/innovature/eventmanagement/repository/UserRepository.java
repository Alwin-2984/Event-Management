...
public interface UserRepository extends JpaRepository<User, Long> {
     ...
     @Query("SELECT COUNT(u) FROM User u WHERE " + "((:status = 0 AND u.status in (0,1)) OR "
               + "(:status = 1 AND u.status = 0) OR " + "(:status = 2 AND u.status = 1)) "
     ...
     @Query("SELECT u FROM User u WHERE " + "((:status = 0 AND u.status in (0,1)) OR "
               + "(:status = 1 AND u.status = 0) OR " + "(:status = 2 AND u.status = 1)) "
               + "AND (LOWER(u.name) LIKE %:name% OR LOWER(u.email) LIKE %:email%)")
